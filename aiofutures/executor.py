...
"""
import asyncio
from concurrent.futures import Executor, Future, ThreadPoolExecutor, InvalidStateError as _InvalidStateError
...
class AsyncExecutorClosed(Exception):
    pass
...
class AsyncExecutor(Executor):
    ...
    def shutdown(self, wait: bool = True, cancel_futures: bool = False) -> None:
        ...
        self._stopped = True
        # process it in self._thread to keep thread safety
        self._submit(self._stop, wait, cancel_futures)
    ...
    async def _stop(self, wait: bool = True, cancel_futures: bool = False):
        if cancel_futures:
            self.cancel_futures()

        if wait:
            if self._loop._scheduled:
                latest = max(self._loop._scheduled, key=lambda x: x.when())
                self._loop.call_at(latest.when() + 1, self._close)
            else:
                self._loop.call_soon(self._close)
            return

        for task in self.tasks:
            with suppress(_InvalidStateError):
                task.set_exception(AsyncExecutorClosed())
    ...
    def _close(self) -> None:
        ...
        if self._thread_executor:
            # All parent async tasks already finished here
